...
def extract(args: Namespace) -> str:
    ...
    out_path = os.path.join(out_path, basename+".png")
...
def main() -> None:
    ...
    path = extract(args)
    img = teaser()
    origin_image = cv2.imread(args.img_path)
    height, width, channels = origin_image.shape
    # Reduce the resolution
    new_size = (width, height)  # Set the desired resolution
    resized_image = img.resize(new_size)

    # Compress the image
    resized_image.save(path, optimize=True, quality=80)
